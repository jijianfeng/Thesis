...
<div class="container" style="background:white">
	<form id="form" action="${__url__}" method="post" enctype="multipart/form-data" >
		<table align="center" class="form-table" style="font-size:15px" bgcolor="white">
			<tbody>
				<tr>
					<input value="${thesis.id}" type="hidden" name="id" />
					<td><label>所属学院：</label>
					<td>
						...
						<c:if test="${thesis.collegeId!=null}"><option value="${thesis.collegeId}" selected="selected"></option></c:if>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						url: '${z:u('public/majorlist_nodefault')}?collegeId=${thesis.collegeId}',
						required:true,
						method:'post',
						editable:true,
						}">
						<c:if test="${thesis.majorId!=null}"><option value="${thesis.majorId}" selected="selected"></option></c:if>
						</select>
					</td>
				</tr>							
				<tr>
					<td><label>论文名称：</label>
					<td><input class="jq-validatebox" value="${thesis.thesisName}" style="width:200px" type="text" id="thesisNamea" name="thesisName" placeholder="输入论文名称" data-options="required:true" />
					</td>
				</tr>
				<tr>
					<td><label>论文序号：</label>
					<td><input class="jq-validatebox" value="${thesis.thesisCode}" style="width:200px" type="text" id="thesisCodea" name="thesisCode" placeholder="输入论文序号" data-options="required:true" />
					</td>
				</tr>
				<tr>
					<td><label>论文类型：</label>
				...
				<tr>
					<td><label>重新上传：</label></td>
					<td><input type="file" id="zip" name ="file"></td>
					<td><button type="submit"  class="btn btn-info btn-sm">保存</button></td>
