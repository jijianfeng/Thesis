	...
	<div data-options="region:'north',border:false">
        <div id="grid-toolbar" class="clearfix p5">
        	<c:if test="${sessionScope.userType==0}">
            	<a id="addCollege" class="btn btn-sm btn-success" ><i class="icon icon-add"></i>增加学院负责人</a>
            	<a id="addColleges" class="btn btn-sm btn-success" ><i class="icon icon-add"></i>批量增加</a>
            </c:if>
            <c:if test="${sessionScope.userType==1}">
            	<a id="addUser" class="btn btn-sm btn-success" ><i class="icon icon-add"></i>增加专家</a>
            	<a id="addUsers" class="btn btn-sm btn-success" ><i class="icon icon-add"></i>批量增加</a>
            </c:if>
            <c:if test="${sessionScope.userType==3}">
            	<a id="addUserAdmin" class="btn btn-sm btn-success" ><i class="icon icon-add"></i>增加专属专家</a>
            	<a id="addUsersAdmin" class="btn btn-sm btn-success" ><i class="icon icon-add"></i>批量增加</a>
            </c:if>
        	<span>所属大学:</span>
...
<!-- 		    <button class="btn btn-xs btn-primary" id="look_university" type="button">查找</button> -->
            ...
            <a id="messEdit" class="btn btn-sm btn-success"><i class="icon icon-edit"></i>编辑</a>
        	...
        	<c:if test="${sessionScope.userType==3}">
<!--         	<a id="add" class="btn btn-sm btn-success" href="${z:u('public/register')}"><i class="icon icon-add"></i>增加</a> -->
            <a id="edit" class="btn btn-sm btn-info"><i class="icon icon-edit"></i>设置备注</a>
	...
	<div data-options="region:'center',border:false">
		<table class="jq-datagrid" fit="true" id="list" data-options="{
			...
			columns: [[
				...
				<c:if test="${sessionScope.userType==3}">
				{field:'userPassword',title:'密码',width:100,sortable:'true'},
				</c:if>
				{field:'nickName',title:'用户名',width:100,sortable:'true'},
				<c:if test="${sessionScope.userType==3}">
				{field:'userRemark',title:'备注',width:100,sortable:'true'},
				</c:if>
				...
				{field:'userSex',title:'性别',width:100,sortable:'true'},
 				...
 				<c:if test="${sessionScope.userType==3}">
 				{field:'isAdmin',title:'属于管理员',width:100,formatter:App.statusFmt,sortable:'true'},
 				</c:if>
			]]}">
		</table>
	</div>
