...
<!--             <a id="download" class="btn btn-sm btn-info"><i class="icon icon-edit"></i>下载论文</a> -->
	...
	<div data-options="region:'center',border:false">
		<table class="jq-datagrid" fit="true" id="list_public" data-options="{
			url: '${z:u('thesis/list_public_admin')}',
			method:'post',
			singleSelect:false,
			...
			columns: [[
				...
				{field:'researchDirection',title:'研究方向',width:100 ,sortable:'true',sortable:'true'},
				{field:'studyType',title:'攻读方式',width:100 ,sortable:'true',sortable:'true'},
  			    ...
  			    <c:if test="${sessionScope.userType==3||sessionScope.userType==4}">
 				{field:'isShortcut',title:'是否选择一键送审',width:100,formatter:App.statusFmt,sortable:'true'},
 				</c:if> 
			]]}">
		</table>
	</div>
...
<script type="text/javascript">
	...
	$("#upAdmin").on("click", function () {
		...
		if(row.length == 0){
			...
			var ids=strIds.join(",");
	        App.popup('${z:u("/thesis/list_up_admin_public")}?id='+ids, {
	            title: "送审",
	            width: 400
	        });
		}
