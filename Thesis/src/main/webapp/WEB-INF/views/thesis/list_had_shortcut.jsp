...
<!--             <a id="sendshortcut" class="btn btn-sm btn-success"><i class="icon icon-edit"></i>一键送审</a> -->
            ...
            <a id="downloadComments" class="btn btn-sm btn-info"><i class="icon icon-edit"></i>下载论文评阅书</a>
...
<!--             </c:if> -->
	...
	<div data-options="region:'center',border:false">
		<table class="jq-datagrid" fit="true" id="thesisList" data-options="{
 			url: '${z:u('thesis/list_had_shortcut')}', 
			method:'post',
			singleSelect:false,
			...
			columns: [[
				...
				{field:'studyType',title:'攻读方式',width:100 ,sortable:'true'},
 				...
 				{field:'requireNumber',title:'需要评审次数',width:100 ,sortable:'true'},
...
// 		var status = $("#status").combobox("getValue");
		$("#thesisList").datagrid({
			url:'${z:u("/thesis/list_had_shortcut")}?thesisName='+thesisName+"&thesisCode="+thesisCode+"&timeStart="+timeStart+"&timeEnd="+timeEnd+"&collegeId="+collegeId+"&majorId="+majorId
		});
	});
...
// 		alert(ids);
	...
	$("#sendpublic").click(function(){
		...
		if(row == null){
			...
			App.popup('${z:u("/thesis/list_send_public")}?id='+row.id, {
				title: "送审到公共平台",
				width: 350,
				height: 260
			});
...
// 		alert(ids);
	...
	$("#downloadComments").click(function(){
		...
		if(row.length == 0){
			...
			var ids=strIds.join(",");
			window.location.href = '${z:u("/thesis/download_commentss")}?id='+ids;
		}
	});
...
// 	});
	...
	$("#edit,#delete").click(function(){
		...
		if(row.length != 1){
		...
		}else if(row.sendNumber!=0){
			App.alert("改论文已经被送审！无法被修改","info");
		}
		else{
			...
			if(eleId=="edit"){
	            ...
	            App.ajax('${z:u("/thesis/list_delete")}?id='+row.id,{
	            msg: "确定要删除所选数据？",
	            type: "POST",
	            success : function(data) {
	        		//alert("dada");
	        		//window.location.href = '${z:u("/#1/thesis/list")}';
				},
				error : function(err) {
					alert("错就是错");
				}
...
// 			$('#majorlistwhere').combobox('reload', '${z:u('public/majorlist')}'+"?collegeId="+b);
// 			$('#majorlistwhere').combobox('setValues','0');
// 		}
// 	});
